...
function Tokenizer:isHexadecimalNumberPrefix()
	...
	return self.curChar == "0" and (nextChar == "x" or nextChar == "X")
end

function Tokenizer:isVarArg()
	return self.curChar == "." and self:lookAhead(1) == "." and self:lookAhead(2) == "."
end

function Tokenizer:isComment()
	return self.curChar == "-" and self:lookAhead(1) == "-"
end
...
function Parser:consumeToken(tokenType, tokenValue)
	...
	error(string.format("Expected %s [%s] token, got: %s [%s]", tokenType, tokenValue, token.TYPE, token.Value))
end
...
function Parser:checkCharacter(character, token)
	...
	return token and token.TYPE == "Character" and token.Value == character
end
...
function Parser:consumeIdentifierList()
	...
	while self.currentToken.TYPE == "Identifier" do
		...
		if not self:isComma(self:lookAhead(1)) then
			break
		end
...
function Parser:consumeParameterList()
	...
	while not self:checkCharacter(")") do
		...
		self:consume(1)
		if not self:isComma(self.currentToken) then
			break
		end
		self:consumeToken("Character", ",")
	end
...
function Parser:consumeTableIndex(currentExpression)
	self:consumeToken("Character", ".")
	local indexToken = { TYPE = "String", Value = self.currentToken.Value }
...
function Parser:consumeTable()
	...
	while not self:checkCharacter("}") do
		...
		elseif self.currentToken.TYPE == "Identifier" and self:checkCharacter("=", self:lookAhead(1)) then
...
function Parser:consumeMethodCall(currentExpression)
	...
	local methodIndexNode = {
		TYPE = "TableIndex",
		Index = { TYPE = "String", Value = methodIdentifier },
		Expression = currentExpression,
	}
...
function Parser:parsePrimaryExpression()
	...
	elseif tokenType == "Constant" then
		return { TYPE = "Constant", Value = tokenValue }
	elseif tokenType == "VarArg" then
		return { TYPE = "VarArg", ReturnValueAmount = 1 }
	...
	elseif tokenType == "Character" then
		if tokenValue == "(" then -- Parenthesized expression
			...
			self:consume(1) -- Consume the last token of the expression
			return {
				TYPE = "ParenthesizedExpr", -- Ostensibly-1: fixed bug #1
				Expression = expression,
			}
...
function Parser:parseBinaryExpression(minPrecedence)
	...
	while true do
		...
		expression = { TYPE = "BinaryOperator", Operator = operatorToken.Value, Left = expression, Right = right }
	end
	return expression
end
...
function Parser:consumeExpressions()
	local expressions = { TYPE = "Expressions", self:consumeExpression() }
...
function Parser:parseIf()
	...
	local branches = { TYPE = "IfBranchList", { TYPE = "IfBranch", Condition = ifCondition, CodeBlock = ifCodeBlock } }
	while self:checkKeyword("elseif") do
		...
		local ifBranch = { TYPE = "IfBranch", Condition = elseifCondition, CodeBlock = elseifCodeBlock }
...
function Parser:getNextNode()
	...
	if currentTokenType == "Keyword" then
		if PARSER_STOP_KEYWORDS[currentTokenValue] then
			return
		elseif currentTokenValue == "local" then
			node = self:parseLocal()
		elseif currentTokenValue == "while" then
			node = self:parseWhile()
		elseif currentTokenValue == "repeat" then
			node = self:parseRepeat()
		elseif currentTokenValue == "do" then
			node = self:parseDo()
		elseif currentTokenValue == "return" then
			node = self:parseReturn()
		elseif currentTokenValue == "break" then
			node = self:parseBreak()
		elseif currentTokenValue == "if" then
			node = self:parseIf()
		elseif currentTokenValue == "for" then
			node = self:parseFor()
		elseif currentTokenValue == "function" then
			node = self:parseFunction()
		else
			error("Unsupported keyword: " .. currentTokenValue)
		end
	else
...
function CodeGenerator:processExpressionNode(node, expressionRegister)
	...
	if nodeType == "Number" then
		return self:compileNumberNode(node, expressionRegister)
	elseif nodeType == "String" then
		return self:compileStringNode(node, expressionRegister)
	elseif nodeType == "Function" then
		return self:compileFunctionNode(node, expressionRegister)
	elseif nodeType == "FunctionCall" then
		return self:compileFunctionCallNode(node, expressionRegister)
	elseif nodeType == "Constant" then
		return self:compileConstantNode(node, expressionRegister)
	elseif nodeType == "VarArg" then
		return self:compileVarArgNode(node, expressionRegister)
	elseif nodeType == "TableIndex" then
		return self:compileTableIndexNode(node, expressionRegister)
	elseif nodeType == "Table" then
		return self:compileTableNode(node, expressionRegister)
	elseif nodeType == "Variable" then
		return self:compileVariableNode(node, expressionRegister)
	elseif nodeType == "BinaryOperator" then
		return self:compileBinaryOperatorNode(node, expressionRegister)
	elseif nodeType == "UnaryOperator" then
		return self:compileUnaryOperatorNode(node, expressionRegister)
	elseif nodeType == "ParenthesizedExpr" then
		 -- Ostensibly-1: fixed bug #2
		return self:processExpressionNode(node.Expression, expressionRegister) -- must compile the first part of the expression only on parenthesis.
	end
...
function CodeGenerator:processStatementNode(node)
	...
	elseif nodeType == "BreakStatement" then
		return self:compileBreakStatementNode()
	elseif nodeType == "LocalFunctionDeclaration" then
		return self:compileLocalFunctionDeclarationNode(node)
	elseif nodeType == "FunctionDeclaration" then
		return self:compileFunctionDeclarationNode(node)
	elseif nodeType == "LocalDeclaration" then
		return self:compileLocalDeclarationNode(node)
	elseif nodeType == "NumericForLoop" then
		return self:compileNumericForLoopNode(node)
	elseif nodeType == "GenericForLoop" then
		return self:compileGenericForLoopNode(node)
	elseif nodeType == "ReturnStatement" then
		return self:compileReturnStatementNode(node)
	elseif nodeType == "WhileLoop" then
		return self:compileWhileLoopNode(node)
	elseif nodeType == "RepeatLoop" then
		return self:compileRepeatLoopNode(node)
	elseif nodeType == "DoBlock" then
		return self:compileDoBlockNode(node)
	elseif nodeType == "IfStatement" then
		return self:compileIfStatementNode(node)
	elseif nodeType == "VariableAssignment" then
		return self:compileVariableAssignmentNode(node)
	end
...
local COMPILER_OPCODE_LOOKUP = {
	["MOVE"] = { 0, MODE_iABC },
	["LOADK"] = { 1, MODE_iABx },
	["LOADBOOL"] = { 2, MODE_iABC },
	["LOADNIL"] = { 3, MODE_iABC },
	["GETUPVAL"] = { 4, MODE_iABC },
	["GETGLOBAL"] = { 5, MODE_iABx },
	["GETTABLE"] = { 6, MODE_iABC },
	["SETGLOBAL"] = { 7, MODE_iABx },
	["SETUPVAL"] = { 8, MODE_iABC },
	["SETTABLE"] = { 9, MODE_iABC },
	["NEWTABLE"] = { 10, MODE_iABC },
	["SELF"] = { 11, MODE_iABC },
	["ADD"] = { 12, MODE_iABC },
	["SUB"] = { 13, MODE_iABC },
	["MUL"] = { 14, MODE_iABC },
	...
	["LE"] = { 25, MODE_iABC },
	["TEST"] = { 26, MODE_iABC },
	["TESTSET"] = { 27, MODE_iABC },
	["CALL"] = { 28, MODE_iABC },
	["TAILCALL"] = { 29, MODE_iABC },
	["RETURN"] = { 30, MODE_iABC },
	["FORLOOP"] = { 31, MODE_iAsBx },
	["FORPREP"] = { 32, MODE_iAsBx },
	["TFORLOOP"] = { 33, MODE_iABC },
	["SETLIST"] = { 34, MODE_iABC },
	["CLOSE"] = { 35, MODE_iABC },
	["CLOSURE"] = { 36, MODE_iABx },
	["VARARG"] = { 37, MODE_iABC },
}
